...
### Graph of total medal counts by country 
```{r}
SmallerAthleteCounts<-
  AthleteCounts%>%
  filter(Total > 100)
ggplot(data = SmallerAthleteCounts, aes(x = M, y = F)) + geom_point()  + aes(colour = Nation) + theme(legend.position = "right") + labs(title = "")
```
